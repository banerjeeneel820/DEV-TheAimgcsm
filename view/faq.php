			...
			<ul class="accordion-box">
             <?php 
               foreach($faqArr as $key => $faq){ 
               	if($faq->file_upload_type == "local"){
                    $optional_pdf = USER_UPLOAD_DIR.'faq/'.$faq->optional_pdf;

                    if(strlen($faq->optional_pdf)>0 && file_exists($optional_pdf)) {   
                      $optional_pdf = USER_UPLOAD_URL.'faq/'.$faq->optional_pdf;
                    }else{
                      $optional_pdf = null;	
                    }

                }else{
                   $optional_pdf = $faq->optional_pdf;
                }
             ?>
				<!--Block-->
				<li class="accordion block">
					<div class="acc-btn">
						<div class="icon-outer">
						  <span class="icon icon-plus fa fa-plus accordion_display" data-display="show"></span> 
						  <span class="icon icon-minus fa fa-minus accordion_display" data-display="show"></span>
					    </div>
					    <?=$faq->title?>
				    </div>
