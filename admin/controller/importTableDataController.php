...
$GlobalInterfaceModelObj = new GlobalInterfaceModel();
   ...
   case 'exam_questions':
      ...
      foreach ($finalSpreadSheetArr as $index => $line) {
         if (!empty($line[0]) && !empty($line[1]) && !empty($line[2]) && !empty($line[3]) && !empty($line[4]) && !empty($line[5]) && !empty($line[6])) {
            ...
            $questionOrderData = $GlobalInterfaceModelObj->fetch_Last_Question_Ordering($exam_id);
            ...
            $GlobalInterfaceModelObj->import_Exam_Questions($paramArr);
         } else {
   ...
   case 'city':
      ...
      foreach ($finalSpreadSheetArr as $index => $line) {
         if (!empty($line[0])) {
            ...
            $GlobalInterfaceModelObj->import_Global_City($paramArr);
         }
      }
      unlink($targetPath);
      echo json_encode(array('check' => 'success', 'message' => 'Data import is successfully completed.'));
      ...
      if ($action_permission) {
         ...
         foreach($finalSpreadSheetArr as $index=> $line){ 
            if(!empty($line[0])){  
               ...
               $studentDetailArr = json_decode(json_encode($GlobalInterfaceModelObj->fetch_Global_Single_Student($stu_id)),true);
               ...
               $GlobalInterfaceModelObj->update_student_monthly_course_fees($paramArr);
            }   
         }
         unlink($targetPath);
